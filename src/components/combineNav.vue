<template lang="pug">
  #tpx-basket-bar.row.justify-content-between.align-items-center.tpx.mx-0.py-0.border-bottom-0.topNavBox
    ...
    div.tpx.tpx-bar-container.tpx-clearfix.topNav(id="tpx-basket-bar-inner" :class="{'show' : sideBarShow}")
      ...
      label(@click="$router.push(`/activity/${activityId}`); $store.state.sideBarShow = false" v-if="activityOptn").item.helpCenter.mb-0.align-items-center.activity 抽獎活動
...
export default{
  ...
  created () {
    ...
    // 活動是否開啟
    vm.activityOptn = window.activity_open
  },
  mounted () {
    let vm = this

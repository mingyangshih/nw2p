<template lang="pug">
  .container.my-4
      swiper.swiper(:options='swiperOption')
          ...
          .swiper-button-prev(slot='button-prev').text-primary
          .swiper-button-next(slot='button-next').text-primary
...
export default{
  ...
  computed: {
    ...mapState({
      recommendedPrdouct: state => state.designerItemModules.recommendedPrdouct
    })
  },
  async created () {
    // const vm = this
